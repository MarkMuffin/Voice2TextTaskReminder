...
async def test_task_create_and_get(task_repo):
    task = Task(user_id="u1", title="Test task", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram")
...
async def test_task_list_by_user(task_repo):
    for i in range(3):
        await task_repo.create(Task(user_id="alice", title=f"Task {i}", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
    await task_repo.create(Task(user_id="bob", title="Bob task", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
...
async def test_task_list_by_user_filtered(task_repo):
    t = await task_repo.create(Task(user_id="u2", title="Active", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
    await task_repo.update_status(t.id, TaskStatus.DONE, completed_at=datetime.utcnow())
    await task_repo.create(Task(user_id="u2", title="Active2", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
...
async def test_task_update_status(task_repo):
    task = await task_repo.create(Task(user_id="u3", title="T", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
    updated = await task_repo.update_status(task.id, TaskStatus.DONE, completed_at=datetime.utcnow())
    assert updated.status == TaskStatus.DONE
    assert updated.completed_at is not None


async def test_task_fuzzy_search(task_repo):
    await task_repo.create(Task(user_id="u4", title="Купить молоко", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
...
async def test_reminder_crud(task_repo, reminder_repo):
    task = await task_repo.create(Task(user_id="u5", title="T", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
...
async def test_reminder_list_pending(task_repo, reminder_repo):
    task = await task_repo.create(Task(user_id="u6", title="T", status=TaskStatus.ACTIVE, timezone="UTC", source="telegram"))
    r1 = await reminder_repo.create(Reminder(task_id=task.id, remind_at=datetime(2025, 1, 1, tzinfo=timezone.utc)))
    r2 = await reminder_repo.create(Reminder(task_id=task.id, remind_at=datetime(2025, 2, 1, tzinfo=timezone.utc)))
