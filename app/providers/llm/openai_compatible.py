...
def is_safe_fallback(intent: ParsedIntent) -> bool:
    """Return True for the local parser fallback, not for every unknown intent."""
    return (
        intent.intent == IntentType.UNKNOWN
        and intent.confidence == 0.0
        and intent.requires_confirmation is True
        and intent.clarification_question == _FALLBACK_QUESTION
    )
...
class OpenAICompatibleIntentParser(BaseIntentParser):
    def __init__(
        self,
        api_key: str,
        model: str,
        ...
        *,
        ...
        retry_without_response_format_on_400: bool = True,
        ...
        self.retry_without_response_format_on_400 = retry_without_response_format_on_400
        self.request_timeout = request_timeout

    async def parse(self, text: str, timezone: str = "Europe/Amsterdam") -> ParsedIntent:
        try:
            return await self._parse_remote(text, timezone)
        except Exception as exc:
            logger.warning(
                "%s parse failed (model=%s): %s",
                self.provider_name,
                self.model,
    ...
    async def _parse_remote(self, text: str, timezone: str) -> ParsedIntent:
        if not self.api_key:
            raise IntentParseError(f"{self.provider_name} API key is not configured")
        ...
        async with httpx.AsyncClient(timeout=self.request_timeout) as client:
            ...
            if response.status_code == 400 and self.retry_without_response_format_on_400:
                logger.warning(
                    "%s: response_format rejected (model=%s, HTTP 400), retrying without it",
                    self.provider_name,
                    self.model,
                )
        ...
        if not raw_content or not raw_content.strip():
            raise IntentParseError(
                f"{self.provider_name} returned empty/null content for model={self.model}"
            )

        content = raw_content.strip()
        logger.debug(
            "%s raw response (model=%s, len=%d): %s",
            self.provider_name,
            self.model,
            len(content),
        ...
        try:
            ...
            raise IntentParseError(f"Invalid {self.provider_name} intent response: {exc}") from exc
